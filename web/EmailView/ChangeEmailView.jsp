<%-- 
    Document   : ChangeEmailView
    Created on : Nov 10, 2018, 6:11:15 PM
    Author     : Firsty
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<div style="background-color: #e7fffc; padding: 4px;">
    <div style="max-width:600px; margin: 8px auto;">
        <div style="background-color: white; border-radius: 8px; padding: 8px;">
            <div align="center"><img src="http://icons.iconarchive.com/icons/jozef89/services-flat/256/ecommerce-icon.png" alt="Cart-Commerce"
                                     style="height: 128px; width: 128px;"></div>
            <h1 align="center"><b>Cart-Commerce</b></h1>
            <hr>
            <div id="content" style="padding-left: 24px; padding-right: 24px;">
                <p>Hello there,</p>
                <p>We recently received a request to change the email address for the Cart-Commerce user UserName to example@email.com</p>
                <p>If this is correct, please click on the following link to change it:</p>
                <div align="center" style="margin: 32px;">
                    <a href="http://localhost:8080/Web-Ecommerce/AccountVerify?a=28462b3a477aff1d0b944ffbd22&b=1" target="_blank" style="padding: 16px; background-color: limegreen; border-radius: 8px; text-decoration: none; color: white;">
                        <b>Verify Address</b>
                    </a>
                </div>
                <p>This email has been sent to example@email.com</p>
                <p>Regards,</p>
                <p>Cart-Commerce support team</p>
                <hr>
                <p align="center"><small>You received this email because you requested to change email to Cart-Commerce.<br>
                        If you didn't request to register, you can safely ignore this e-mail.</small></p>
            </div>
        </div>
    </div>
</div>

